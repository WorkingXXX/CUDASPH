...
void ParticleSystem::SetupCUDA()
{
	InitSimulation(gScene);
}

void ParticleSystem::setRender() {

	/*m_creator.CreateShaderProgram("vertex.glsl", "fragment.glsl");
	m_creator.UseShaderProgram();*/
...
void ParticleSystem::Draw(Camera3D& cam, float rad) {
	...
	for (int n = 0; n < this->num_points_; n++) {
		...
		printf("point[%d] = (%.6f, %.6f, %.6f)\n", n, points[n].pos.x, points[n].pos.y, points[n].pos.z);
...
void ParticleSystem::DrawCUDA(Camera3D& camera, float ptcRadius)
{
	glDisable(GL_LIGHTING);
	...
	for (size_t i = 0; i < gHostFldCst.ptcTotalNum; ++i) 
	{
		glPushMatrix();
		glTranslatef(gHostPos[i].x, gHostPos[i].y, gHostPos[i].z);
		//printf("point[%d] = (%.6f, %.6f, %.6f)\n", i, gHostPos[i].x, gHostPos[i].y, gHostPos[i].z);
		//glTranslatef(0.1, 0.1, 0.1);
		glScalef(ptcRadius, ptcRadius, ptcRadius);
		//glColor4f(RED(points[n].clr), GRN(points[n].clr), BLUE(points[n].clr), ALPH(points[n].clr));
		drawSphere();
		glPopMatrix();
	}
